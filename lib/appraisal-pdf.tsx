/**
 * Shared per-appraisal PDF builder. Used by the single-appraisal download route
 * and by the bulk PDF-pack (ZIP) route so the document stays identical in both.
 * Returns the rendered buffer + a safe filename; auth + audit live in the routes.
 */
...
const CHOICE: Record<string, string> = { agree: 'Agree', agree_remarks: 'Agree with remarks', disagree: 'Disagree' };
...
export interface RenderedAppraisalPdf { buffer: Buffer; filename: string; cycleId: number; }

export async function renderAppraisalPdf(appraisalId: string): Promise<RenderedAppraisalPdf | null> {
  const rows = (await sql()`
    ...
    WHERE a.id = ${appraisalId}`) as Record<string, unknown>[];
  const a = rows[0];
  if (!a) return null;
  ...
  const filename = `${String(a.cycle_label).replace(/\s+/g, '-')}-${String(a.emp_code)}-${String(a.full_name).replace(/\s+/g, '-')}.pdf`;
  return { buffer, filename, cycleId: a.cyc_id as number };
}
